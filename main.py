...
def train_model_for_each_movie(list_arr):
    ...
    for i in range(len(list_arr)):
        print('Strating training for movie = ' + str(x))
...
def extract_input_output(input_dict):
    ...
    train_x, test_x, train_y, test_y = train_test_split(stacked_X, encoded_Y, test_size=0.3,
                                                        train_size=0.7, shuffle=True,
                                                        stratify=None)
    # print('after split:' + str(train_x.shape) + str(train_y.shape) + str(test_x.shape) + str(test_y.shape))
    print('Partitioning and splitting data done successfully! ')
...
def label_encoding(input_array):
    out_array = np.zeros((input_array.shape))
    for i in range(input_array.shape[0]):
        if input_array[i] == 'True':
            out_array[i] = 1
        elif input_array[i] == 'False':
            out_array[i] = 0
        else:
            print('Invalid value!')
    return out_array
